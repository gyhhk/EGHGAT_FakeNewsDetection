...
class GHGAT(nn.Module):
    def __init__(self, params):
        super(GHGAT, self).__init__()
        self.para_init()
        self.attention = True
    ...
    def forward(self, x_list, adj_list, all_distance):
        x0 = x_list
        ...
        if not self.lower_attention:
            ...
            x1 = [None for _ in range(self.ntype)]
            x1_in = self.gc1(x0, adj_list, all_distance)
...
class ConcatTransform(nn.Module):
    ...
    def forward(self, X: torch.FloatTensor, Y: torch.FloatTensor):
        ...
        '''
        Y = self.preW(self.dropout(Y))
        concatVector = torch.cat([X, Y], dim=1)
...
class MatchingTransform(nn.Module):
    ...
    def forward(self, X: torch.FloatTensor, Y: torch.FloatTensor):
        ...
        Y = self.preW(self.dropout(Y))
        if self.SIMPLE:
            matchingVector = torch.cat([X - Y, X.mul(Y)], dim=1)
        else:
            matchingVector = torch.cat([X, Y, X - Y, X.mul(Y)], dim=1)
