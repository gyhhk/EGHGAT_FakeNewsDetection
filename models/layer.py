        ...
        for t1 in range(self.ntype):
            ...
            for t2 in range(self.ntype):
                if len(adj_list[t1][t2]._values()) == 0:
                    zeros = torch.zeros(adj_list[t1][t2].shape[0], self.out_features, device=self.bias.device,
                                        dtype=self.weights[0].dtype)
                    x_t1.append(zeros)
                    continue
                if self.bias is not None:
                    x_t1.append(self.att_list[t1](h[t1], h[t2], adj_list[t1][t2],
                                                  all_distance[dis_bias[t1]:dis_bias[t1 + 1],
                                                  dis_bias[t2]:dis_bias[t2 + 1]]) + self.bias)
                else:
                    x_t1.append(self.att_list[t1](h[t1], h[t2], adj_list[t1][t2],
                                                  all_distance[dis_bias[t1]:dis_bias[t1 + 1],
                                                  dis_bias[t2]:dis_bias[t2 + 1]]))
            outputs.append(x_t1)
...
class Attention_InfLevel(nn.Module):
    ...
    def forward(self, input1, input2, adj, distance):
        ...
        attention = torch.where(distance >= 0, e, zero_vec)
        e = attention
        for i in range(2, 6):
            att = torch.where(distance == i, e, zero_vec)
            attention = torch.where(att > 0, (1 / i) * e, attention)
            attention = torch.where(att < 0, i * e, attention)
