...
class Evaluator:
    ...
    def _evaluate_aux(self, model, data_loader):
        ...
        for inputs in tqdm(data_loader):
            with torch.no_grad():
                try:
                    documents, ent_desc, doc_lens, ent_lens, y_batch, adj_lists, feature_lists, sentPerDoc, entiPerDoc, all_distance = \
    ...
    def evaluate(self):
        ...
        if self.params.ntags == 2:
            ...
            prec_mac, recall_mac, f1_mac, _ = precision_recall_fscore_support(all_actual, all_predicted,
                                                                              average='macro')
            prec_mic, recall_mic, f1_mic, _ = precision_recall_fscore_support(all_actual, all_predicted,
                                                                              average='micro')
            ...
            print(
                "Latex: {:5.2f} & {:5.2f} & {:5.2f} & {:5.2f}".format(accuracy * 100, prec_mac * 100, recall_mac * 100,
                                                                      f1_mac * 100))
            print("----------------------------------------------------------------------")
