...
class Utils:
    ...
    def train(self, save_plots_as, pretrained_emb=None):
        model = Classifier(self.params, vocab_size=len(self.data_loader.w2i), pte=pretrained_emb)
        if self.params.HALF:
            model.half()
        ...
        optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=self.params.lr,
                               weight_decay=self.params.weight_decay, eps=1e-4)
